...
def metav2d(tdir):
    args = {"private": True, "path": tdir,
            "announce": "http://announce.com/announce",
            "source": "tracker", "comment": "content details and purpose"}
    torrent = TorrentFileV2(**args)
    torrent.assemble()
    outfile, meta = torrent.write()
    yield outfile, meta
...
def test_v2_info_keys_file(metav2f):
    outfile, meta = metav2f
    for key in ["length", "piece length",  "meta version",
                "file tree", "name", "private"]:
        assert key in meta["info"]
    assert os.path.exists(outfile)

def test_v2_info_keys_dir(metav2d):
    outfile, meta = metav2d
    for key in ["piece length",  "meta version", "file tree",
